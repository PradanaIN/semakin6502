...
const hash_1 = require("../src/shared/security/hash");
const status_constants_1 = require("../src/shared/constants/status.constants");
const holidays_1 = require("../src/shared/utils/holidays");
...
async function main() {
    await prisma.role.createMany({
        data: [
            { id: (0, ulid_1.ulid)(), name: "admin" },
            { id: (0, ulid_1.ulid)(), name: "pimpinan" },
            { id: (0, ulid_1.ulid)(), name: "ketua" },
            { id: (0, ulid_1.ulid)(), name: "anggota" },
        ],
        skipDuplicates: true,
    });
    ...
    await prisma.team.createMany({
        data: teamNames.map((n) => ({ id: (0, ulid_1.ulid)(), namaTim: n })),
        skipDuplicates: true,
    });
    const teams = await prisma.team.findMany({
        where: { namaTim: { in: teamNames } },
    });
    const teamMap = new Map(teams.map((t) => [t.namaTim, t.id]));
    const kegiatanByTeam = {
        Umum: [
            "SAKIP",
            "SPI",
            "Keuangan",
        ...
        for (const nama of kegiatan) {
            ...
            if (!existing) {
                await prisma.masterKegiatan.create({
                    data: { id: (0, ulid_1.ulid)(), teamId, namaKegiatan: nama },
                });
            }
        }
    }
    ...
    function getWeekRanges(year, monthIndex, weeks) {
        if (weeks)
            return weeks;
        const daysInMonth = new Date(Date.UTC(year, monthIndex + 1, 0)).getUTCDate();
        const firstDay = new Date(Date.UTC(year, monthIndex, 1)).getUTCDay();
        const firstWeekEnd = firstDay === 0 ? 7 : 7 - firstDay + 1;
        const result = [{ start: 1, end: firstWeekEnd }];
    ...
    for (const m of members) {
        if (m.isLeader)
            leaderByTeam.set(m.teamId, m.userId);
    }
    const holidays = new Set((0, holidays_1.getHolidays)(2025));
    const tambahanRows = [];
    const laporanTambahanRows = [];
    const penugasanRows = [];
    const laporanPenugasanRows = [];
    const selesaiPenugasanIds = new Set();
        ...
        for (const info of months) {
            ...
            for (let w = 1; w <= weeksToProcess; w++) {
                ...
                for (let i = 0; i < taskCount; i++) {
                    ...
                    const status = info.bulan === "7" || info.bulan === "8"
                        ? status_constants_1.STATUS.SELESAI_DIKERJAKAN
                        : status_constants_1.STATUS.BELUM;
                    const row = {
                        id,
                        kegiatanId: k.id,
                        pegawaiId: m.userId,
                    ...
                    arr.push(row);
                    if (status === status_constants_1.STATUS.SELESAI_DIKERJAKAN)
                        selesaiPenugasanIds.add(id);
                }
                ...
                const statusOptions = [
                    status_constants_1.STATUS.BELUM,
                    status_constants_1.STATUS.SEDANG_DIKERJAKAN,
                    status_constants_1.STATUS.SELESAI_DIKERJAKAN,
                ];
                const tStatus = info.bulan === "7" || info.bulan === "8"
                    ? status_constants_1.STATUS.SELESAI_DIKERJAKAN
                    : statusOptions[randomInt(0, statusOptions.length - 1)];
                const kTambahan = masters[randomInt(0, masters.length - 1)];
                const tambahanId = (0, ulid_1.ulid)();
                const tambahan = {
                    id: tambahanId,
                    nama: kTambahan.namaKegiatan,
                    tanggal: date.toISOString(),
                    status: tStatus,
                    buktiLink: tStatus === status_constants_1.STATUS.BELUM ? undefined : `${BASE_URL}/bukti`,
                    userId: m.userId,
    ...
    const eligibleMembers = members.filter((m) => m.user.role !== "admin" && m.user.role !== "pimpinan");
