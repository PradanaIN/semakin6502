...
describe('MonitoringService aggregated', () => {
    ...
    it('harianAll aggregates and includes users without reports', async () => {
        prisma.user.findMany.mockResolvedValue([
            { id: '1', nama: 'A' },
            { id: '2', nama: 'B' },
            { id: '3', nama: 'C' },
        ...
        prisma.laporanHarian.findMany.mockResolvedValue([
            ...
            { pegawaiId: '1', status: status_constants_1.STATUS.BELUM, pegawai: { nama: 'A' } },
            {
                pegawaiId: '1',
                status: status_constants_1.STATUS.SELESAI_DIKERJAKAN,
                buktiLink: 'link',
        ...
        expect(res).toEqual([
            ...
            { userId: '3', nama: 'C', selesai: 0, total: 0, persen: 0 },
        ]);
    });
    ...
    it('mingguanAll aggregates and sorts', async () => {
        ...
        prisma.laporanHarian.findMany.mockResolvedValue([
            { pegawaiId: '2', status: status_constants_1.STATUS.SELESAI_DIKERJAKAN, buktiLink: 'link' },
            { pegawaiId: '1', status: status_constants_1.STATUS.BELUM },
            { pegawaiId: '1', status: status_constants_1.STATUS.SELESAI_DIKERJAKAN, buktiLink: 'link' },
        ]);
    ...
    it('mingguan calculates progress from assignments and marks presence', async () => {
        ...
        prisma.laporanHarian.findMany.mockResolvedValue([
            ...
            { tanggal: new Date('2024-05-01'), status: status_constants_1.STATUS.BELUM },
        ]);
    ...
    it('bulananAll aggregates and passes bulan filter', async () => {
        prisma.penugasan.findMany.mockResolvedValue([
            { pegawaiId: '2', bulan: '1', status: status_constants_1.STATUS.SELESAI_DIKERJAKAN, pegawai: { nama: 'B' } },
            { pegawaiId: '1', bulan: '1', status: status_constants_1.STATUS.BELUM, pegawai: { nama: 'A' } },
            { pegawaiId: '1', bulan: '1', status: status_constants_1.STATUS.SELESAI_DIKERJAKAN, pegawai: { nama: 'A' } },
        ]);
    ...
    it('harianBulan groups by user and counts records', async () => {
        ...
        expect(res.map((u) => u.nama)).toEqual(['A', 'B', 'C']);
    ...
    it('mingguanBulan returns zero weeks for all users when no reports', async () => {
        ...
        const res = await service.mingguanBulan('2024-05-10');
        const zeroWeeks = () => Array.from({ length: 5 }, () => ({ selesai: 0, total: 0, persen: 0 }));
        expect(res).toEqual([
    ...
    it('penugasanBulan aggregates per week', async () => {
        prisma.penugasan.findMany.mockResolvedValue([
            { minggu: 1, status: status_constants_1.STATUS.SELESAI_DIKERJAKAN },
            { minggu: 1, status: status_constants_1.STATUS.BELUM },
            { minggu: 3, status: status_constants_1.STATUS.SELESAI_DIKERJAKAN },
        ]);
        ...
        prisma.user.findMany.mockResolvedValue([
            {
                id: '1',
                nama: 'A',
                role: roles_constants_1.ROLES.ANGGOTA,
                laporan: [{ tanggal: new Date('2024-05-02') }],
            },
            {
                id: '2',
                nama: 'B',
                role: roles_constants_1.ROLES.KETUA,
